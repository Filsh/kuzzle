...
RequestObject.prototype.checkInformation = function () {
  ...
  if (!this.controller) {
    return q.reject(new ResponseObject(this, new BadRequestError('No controller provided for object')));
  }

  // Test if the action is well defined
  if (!this.action) {
    return q.reject(new ResponseObject(this, new BadRequestError('No action provided for object')));
  }
