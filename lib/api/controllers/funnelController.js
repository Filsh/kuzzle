  ...
  return kuzzle.repositories.token.verifyToken(getBearerTokenFromHeaders(requestObject.headers))
    ...
    .then(() => {
      if (!controllers[requestObject.controller] || !controllers[requestObject.controller][requestObject.action] ||
        ...
        return q.reject(new ResponseObject(requestObject, new BadRequestError('No corresponding action ' + requestObject.action + ' in controller ' + requestObject.controller)));
      }
    ...
    .then(isAllowed => {
      if (!isAllowed) {
        if ( context.token.user._id === -1 ) {
          // User anonymous: we throw a 'Unauthozied error' (status 401)
          return q.reject(new ResponseObject(requestObject,
                                                      new UnauthorizedError('Unauthorized action [' +
                                                        requestObject.index + '/' +
                                                        requestObject.collection + '/' +
                                                        requestObject.controller + '/' +
                                                        requestObject.action + '] for anonymous user')));
        } else {
          // User authenticated:  we throw a 'Forbidden error' (status 403)
          return q.reject(new ResponseObject(requestObject,
                                                      new ForbiddenError('Forbidden action [' +
                                                        requestObject.index + '/' +
                                                        requestObject.collection + '/' +
                                                        requestObject.controller + '/' +
